...
/*int main ()
{
	...
	KdTree* tree = new KdTree;// define tree as a local object of the structure KdTree on Heap
  
    for (int i=0; i<points.size(); i++) 
    	tree->insert(points[i],i); //insert each point along with its id in the tree (one at a time)... tree on heap
